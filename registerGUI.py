...
class RegisterDashboard:
    ...
    def ask_register(self):
        ...
        success, payload = self.database_enter()
    ...
    def _show_duplicate_criminal_modal(self, existing_id, existing_name, existing_crime, new_violation, matches=None):
        """Show modal when a duplicate face is detected."""
        matches = matches or []
        modal = Toplevel(self.root)
        modal.transient(self.root)
        modal.grab_set()
        modal.resizable(False, False)
        modal.title("Duplicate Face Detected")
        ...
        Label(
            frame,
            text="This face already exists in the system.",
            bg=self.colors["panel"],
            fg=self.colors["text"],
            font=("Segoe UI", 10),
        ).place(x=18, y=44)
        ...
        Label(
            frame,
            text="Existing Criminal Information:",
            bg=self.colors["panel"],
            fg=self.colors["accent"],
            font=("Segoe UI Semibold", 10),
        ).place(x=18, y=74)
        ...
        Label(
            frame,
            text=f"Criminal ID: {existing_id}",
            bg=self.colors["panel"],
            fg=self.colors["text"],
            font=("Segoe UI", 9),
        ).place(x=30, y=104)
        ...
        if matches:
            collision_lines = [f"- ID {m[0]} | {m[1]} | {m[2]}" for m in matches[:3]]
            Label(
                frame,
                text="Matching records for this face:\n" + "\n".join(collision_lines),
                bg=self.colors["panel"],
                fg="#FFAD42",
                font=("Segoe UI", 9),
                justify=LEFT,
        ...
        history_text = self._get_violation_history_text(existing_id, limit=3)
        Label(
            frame,
        ...
        violation_btn.place(x=299, y=290)
        ...
        if result["value"]:
            ...
            self._show_notification(
                ...
                message=(
                    f"Existing record was kept for {existing_name} (ID {existing_id}).\n"
    ...
    def _check_duplicate_face(self):
        ...
        Returns: (exists: bool, criminal_id: int or None, criminal_info: tuple or None)
        """
        ...
        try:
            ...
            if len(face_distances) > 0:
                ...
                if matched_indices:
                    ...
                    if matched_rows:
                        return True, matched_rows[0][0], matched_rows
